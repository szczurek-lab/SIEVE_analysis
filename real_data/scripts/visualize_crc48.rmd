...
SIEVE_PREFIX <- "../crc48/improved"
ALT_PREFIX <- "../crc48/all_methods"

CELL_ORIGIN <- list(
    c("^XR", "Adenomatous polyps"),
    c("^1", "Cancer tissue 1"),
    c("^2", "Cancer tissue 2")
  )
...
CELLPHY_TREE <- paste(ALT_PREFIX, "cellphy", "crc48.raxml.bestTree.nexus", sep = "/")
SCIPHI_TREE <- paste(ALT_PREFIX, "sciphi", "crc48", "best_index", "tree.nexus", sep = "/")
...
MONOVAR <- "../crc48/crc48_monovar.vcf.gz"
...
tree.plot <- plot.nodes(
  ...
  tip.label.linetype = tip.label.linetype,
  tip.label.linesize = tip.label.linesize,
  tip.label.offset = 0.00000006,
  tip.label.size = tip.label.size,
  tip.label.legend.title = tip.label.legend.title,
  tip.label.color = tip.label.color,
  x.limit = c(0, 0.0000027),
  legend.spacing = legend.spacing,
  legend.pos = c(0, 1),
  legend.just = c("left", "top"),
  legend.direction = "vertical",
...
tree.plot.1 <- plot.genes(
  ...
  text.repel.box.padding = unit(0.7, "lines"),
...
out.gt.hm.width <- 70 # mm
out.gt.hm.height <- 40 # mm
...
# #         sieve.ado.hm.column.order[350L],
